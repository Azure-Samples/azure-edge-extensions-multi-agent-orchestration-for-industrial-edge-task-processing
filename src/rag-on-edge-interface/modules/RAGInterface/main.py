...
def check_processed_result(request_id):
    ...
    return jsonify({'status': 'request_id not found'})
...
def subscribe():
    subscriptions = [{
        ...
        'topic': 'vdb_output_topic',
        'route': 'vdb_output_topic_handler'
    }]
    print('Dapr pub/sub is subscribed to: ' + json.dumps(subscriptions))
    return jsonify(subscriptions)

# Dapr subscription in /dapr/subscribe
@app.route('/vdb_output_topic_handler', methods=['POST'])
def orders_subscriber():
    event = from_http(request.headers, request.get_data())
    print('Subscriber received : %s' % event.data['vdb_result'], flush=True)
    user_query = str(event.data['user_query'])
    vdb_result = str(event.data['vdb_result'])
    request_id = event.data['request_id']

    pending_requests[request_id]["processed_result"] = vdb_result
